...
class DistanceBlock:
                ...
                with tf.device("/gpu:%d" % gpu_idx):
                    distances_split.append(
                        batch_pairwise_distances(
                            self._features_batch1, features_split2[gpu_idx]
                        )
                    )
    ...
    def pairwise_distances(self, U, V):
        ...
        return self._distance_block.eval(
...
class ManifoldEstimator:
    ...
    def evaluate(self, eval_features, return_realism=False, return_neighbors=False):
        ...
        distance_batch = np.zeros(
            [self.row_batch_size, num_ref_images], dtype=np.float16
        )
        ...
        for begin1 in range(0, num_eval_images, self.row_batch_size):
            ...
            realism_score[begin1:end1] = self.D[
                nearest_indices[begin1:end1], 0
            ] / np.min(distance_batch[0 : end1 - begin1, :], axis=1)
    ...
    def _evaluate(self, Gs, Gs_kwargs, num_gpus):
        ...
        if os.path.isfile(cache_file):
            ...
            for idx, images in enumerate(
                ...
                ref_features[begin:end] = feature_net.run(
        ...
        for begin in range(0, self.num_images, minibatch_size):
            ...
            eval_features[begin:end] = np.concatenate(tflib.run(result_expr), axis=0)[
                : end - begin
            ]

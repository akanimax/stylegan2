...
from .. import util
from . import autosummary, tfutil
    ...
    def _get_device(self, device_name: str):
        ...
        with tfutil.absolute_name_scope(self.scope + "/Devices"), tf.device(
            ...
            if device_name not in self._shared_optimizers:
                optimizer_name = self.scope.replace("/", "_") + "_opt%d" % len(
                    self._shared_optimizers
                )
    ...
    def apply_updates(self, allow_no_op: bool = False) -> tf.Operation:
        ...
        for device_idx, device in enumerate(self._devices.values()):
            with tfutil.absolute_name_scope(
                self.scope + "/Clean%d" % device_idx
            ), tf.device(device.name):
        ...
        if len(self._devices) > 1:
            with tfutil.absolute_name_scope(self.scope + "/Broadcast"), tf.device(None):
                for all_vars in zip(
                    *[device.grad_clean.keys() for device in self._devices.values()]
                ):
                    if len(all_vars) > 0 and all(
                        dim > 0 for dim in all_vars[0].shape.as_list()
                    ):  # NCCL does not support zero-sized tensors.
                        all_grads = [
                            device.grad_clean[var]
                            for device, var in zip(self._devices.values(), all_vars)
                        ]
                        all_grads = nccl_ops.all_sum(all_grads)
                        for device, var, grad in zip(
                            self._devices.values(), all_vars, all_grads
                        ):
        ...
        for device_idx, device in enumerate(self._devices.values()):
            with tfutil.absolute_name_scope(
                ...
                if self.minibatch_multiplier is None:
                    ...
                    count_reset_op = lambda: tf.assign(
                        device.grad_acc_count, tf.zeros([])
                    )
                ...
                # Adjust loss scaling.
                if self.use_loss_scaling:
                    ls_inc_op = lambda: tf.assign_add(
                        device.loss_scaling_var, self.loss_scaling_inc
                    )
                    ls_dec_op = lambda: tf.assign_sub(
    ...
    def apply_gradients(self, grads_and_vars):
        with tf.name_scope(self.name):
            ...
            for grad, var in grads_and_vars:
                ...
                update_ops += [
                    tf.assign(m_var, m_new),
                    tf.assign(v_var, v_new),
                    tf.assign_sub(var, var_delta),
                ]

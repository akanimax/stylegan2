...
class Network:
    ...
    def _init_graph(self) -> None:
        ...
        with tfutil.absolute_variable_scope(
            ...
            with tf.control_dependencies(
                ...
                self.input_templates = [
                    tf.placeholder(tf.float32, name=name) for name in self.input_names
                ]
    ...
    def get_output_for(
        ...
        with tfutil.absolute_variable_scope(self.scope, reuse=True), tf.name_scope(
            self.name
        ):
            assert tf.get_variable_scope().name == self.scope
            ...
            for expr, name, shape in zip(in_expr, self.input_names, self.input_shapes):
                if expr is not None:
                    ...
                    expr = tf.zeros(
                        [tf.shape(valid_inputs[0])[0]] + shape[1:], name=name
                    )
    ...
    def get_var_local_name(self, var_or_global_name: Union[TfExpression, str]) -> str:
        ...
        global_name = (
            var_or_global_name
            if isinstance(var_or_global_name, str)
            else var_or_global_name.name
        )
    ...
    def find_var(self, var_or_local_name: Union[TfExpression, str]) -> TfExpression:
        ...
        return (
            self.vars[var_or_local_name]
            if isinstance(var_or_local_name, str)
            else var_or_local_name
        )
    ...
    def set_var(
        self,
        var_or_local_name: Union[TfExpression, str],
        new_value: Union[int, float, np.ndarray],
    ) -> None:
    ...
    def __setstate__(self, state: dict) -> None:
        ...
        self.reset_own_vars()
        tfutil.set_vars(
            {self.find_var(name): value for name, value in state["variables"]}
        )
    ...
    def convert(
        self, new_func_name: str, new_name: str = None, **new_static_kwargs
    ) -> "Network":
                ...
                for gpu in range(num_gpus):
                    with tf.device("/gpu:%d" % gpu):
                        ...
                        if input_transform is not None:
                            ...
                            in_gpu = (
                                [in_gpu]
                                if tfutil.is_tf_expression(in_gpu)
                                else list(in_gpu)
                            )
                        ...
                        if output_transform is not None:
                            ...
                            out_gpu = (
                                [out_gpu]
                                if tfutil.is_tf_expression(out_gpu)
                                else list(out_gpu)
                            )
    ...
    def list_layers(self) -> List[Tuple[str, TfExpression, List[TfExpression]]]:
        ...
        def recurse(scope, parent_ops, parent_vars, level):
            ...
            if any(
                p in scope
                for p in ["/Shape", "/strided_slice", "/Cast", "/concat", "/Assign"]
            ):
                return
            ...
            cur_ops = [
                ...
                if op.name.startswith(global_prefix) or op.name == global_prefix[:-1]
            ]
            cur_vars = [
                (name, var)
                for name, var in parent_vars
            ...
            contains_direct_ops = any(
                "/" not in op.name[len(global_prefix) :]
                and op.type not in ["Identity", "Cast", "Transpose"]
                for op in cur_ops
            )
            if (level == 0 or not contains_direct_ops) and (
                len(cur_ops) + len(cur_vars)
            ) > 1:
                visited = set()
    ...
    def print_layers(
        self, title: str = None, hide_layers_with_no_params: bool = False
    ) -> None:
        """Print a summary table of the network structure."""
        rows = [
            [
                title if title is not None else self.name,
                "Params",
                "OutputShape",
                "WeightShape",
            ]
        ...
        for layer_name, layer_output, layer_trainables in self.list_layers():
            ...
            if not hide_layers_with_no_params or num_params != 0:
                ...
                rows += [
                    [layer_name, num_params_str, output_shape_str, weight_shape_str]
                ]
...
def _handle_legacy_output_transforms(output_transform, dynamic_kwargs):
    ...
    if _print_legacy_warning:
        ...
        print()
        print(
            "WARNING: Old-style output transformations in Network.run() are deprecated."
        )
        print(
            "Consider using 'output_transform=dict(func=tflib.convert_images_to_uint8)'"
        )
    ...
    new_transform = {
        kwarg: new_kwargs.pop(kwarg)
        for kwarg in legacy_kwargs
        if kwarg in dynamic_kwargs
    }
    new_transform["func"] = _legacy_output_transform_func
    return new_transform, new_kwargs


def _legacy_output_transform_func(

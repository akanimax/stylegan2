...
def get_path_from_template(
    path_template: str, path_type: PathType = PathType.AUTO
) -> str:
...
def _populate_run_dir(submit_config: SubmitConfig, run_dir: str) -> None:
    ...
    ) and submit_config.local.do_not_copy_source_files:
        return
    ...
    files += [
        (
            os.path.join(dnnlib_module_dir_path, "submission", "internal", "run.py"),

...
def project_image(proj, targets, png_prefix, num_snapshots):
    snapshot_steps = set(
        proj.num_steps
        - np.linspace(0, proj.num_steps, num_snapshots, endpoint=False, dtype=int)
    )
    misc.save_image_grid(targets, png_prefix + "target.png", drange=[-1, 1])
    proj.start(targets)
    while proj.get_cur_step() < proj.num_steps:
        print(
            "\r%d / %d ... " % (proj.get_cur_step(), proj.num_steps), end="", flush=True
        )
        proj.step()
        if proj.get_cur_step() in snapshot_steps:
            misc.save_image_grid(
                proj.get_images(),
                png_prefix + "step%04d.png" % proj.get_cur_step(),
                drange=[-1, 1],
            )
...
Run 'python %(prog)s <subcommand> --help' for subcommand help.""",
    ...
    project_generated_images_parser = subparsers.add_parser(
        "project-generated-images", help="Project generated images"
    )
    project_generated_images_parser.add_argument(
        "--network", help="Network pickle filename", dest="network_pkl", required=True
    )
    project_generated_images_parser.add_argument(
        "--seeds", type=_parse_num_range, help="List of random seeds", default=range(3)
    )
    project_generated_images_parser.add_argument(
        "--num-snapshots",
        type=int,
    ...
    project_real_images_parser = subparsers.add_parser(
        "project-real-images", help="Project real images"
    )
    project_real_images_parser.add_argument(
        "--network", help="Network pickle filename", dest="network_pkl", required=True
    )
    project_real_images_parser.add_argument(
        "--data-dir", help="Dataset root directory", required=True
    )
    project_real_images_parser.add_argument(
        "--dataset", help="Training dataset", dest="dataset_name", required=True
    )
    project_real_images_parser.add_argument(
        "--num-snapshots",
        type=int,

...
def create_mnist(tfrecord_dir, mnist_dir):
    ...
    images = np.pad(
        images, [(0, 0), (0, 0), (2, 2), (2, 2)], "constant", constant_values=0
    )
        ...
        with TFRecordExporter(tfrecord_dir, max_images, print_progress=False) as tfr:
            for idx, (_key, value) in enumerate(txn.cursor()):
                try:
                    ...
                    print(
                        "\r%d / %d => %d " % (idx + 1, total_images, tfr.cur_images),
                        end="",
                    )
...
def create_from_hdf5(tfrecord_dir, hdf5_filename, shuffle):
    ...
    with h5py.File(hdf5_filename, "r") as hdf5_file:
        hdf5_data = max(
            [value for key, value in hdf5_file.items() if key.startswith("data")],
        ...
        with TFRecordExporter(tfrecord_dir, hdf5_data.shape[0]) as tfr:
            order = (
                ...
                if shuffle
                else np.arange(hdf5_data.shape[0])
            )
            for idx in range(order.size):
...
def execute_cmdline(argv):
    ...
    p.add_argument(
        "--num_images",
        help="Number of composite images to create (default: 1000000)",
        type=int,
        default=1000000,
    )
    p.add_argument(
        "--random_seed", help="Random seed (default: 123)", type=int, default=123
    )

    p = add_command(
        "create_cifar10",
        "Create dataset for CIFAR-10.",
    ...
    p = add_command(
        "create_lsun",
        "Create dataset for single LSUN category.",
    ...
    p.add_argument(
        "--resolution", help="Output resolution (default: 256)", type=int, default=256
    )
    p.add_argument(
        "--max_images",
    ...
    p = add_command(
        "create_lsun_wide",
        "Create LSUN dataset with non-square aspect ratio.",
    ...
    p.add_argument(
        "--height", help="Output height (default: 384)", type=int, default=384
    )
    p.add_argument(
        "--max_images",
        help="Maximum number of images (default: none)",

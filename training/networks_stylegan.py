...
def apply_noise(x, noise_var=None, randomize_noise=True):
    ...
    with tf.variable_scope("Noise"):
        ...
        weight = tf.get_variable(
            "weight", shape=[x.shape[1].value], initializer=tf.initializers.zeros()
        )
...
def minibatch_stddev_layer(x, group_size=4, num_new_features=1):
    with tf.variable_scope("MinibatchStddev"):
        ...
        y = tf.reshape(
            x, [group_size, -1, num_new_features, s[1] // num_new_features, s[2], s[3]]
...
def G_style(
    ...
    **kwargs
):  # Arguments for sub-networks (G_mapping and G_synthesis).
    ...
    if "synthesis" not in components:
        components.synthesis = tflib.Network(
            "G_synthesis", func_name=G_synthesis, **kwargs
        )
    num_layers = components.synthesis.input_shape[1]
    dlatent_size = components.synthesis.input_shape[2]
    if "mapping" not in components:
        components.mapping = tflib.Network(
            "G_mapping", func_name=G_mapping, dlatent_broadcast=num_layers, **kwargs
        )
    ...
    if dlatent_avg_beta is not None:
        with tf.variable_scope("DlatentAvg"):
            ...
            update_op = tf.assign(
                dlatent_avg, tflib.lerp(batch_avg, dlatent_avg, dlatent_avg_beta)
            )
...
def G_mapping(
    ...
    **_kwargs
    ...
    act, gain = {"relu": (tf.nn.relu, np.sqrt(2)), "lrelu": (leaky_relu, np.sqrt(2))}[
    ...
    for layer_idx in range(mapping_layers):
        with tf.variable_scope("Dense%d" % layer_idx):
            ...
            x = dense(
                x, fmaps=fmaps, gain=gain, use_wscale=use_wscale, lrmul=mapping_lrmul
            )
...
def G_synthesis(
    ...
    **_kwargs
    ...
    lod_in = tf.cast(
        tf.get_variable("lod", initializer=np.float32(0), trainable=False), dtype
    )
    ...
    with tf.variable_scope("4x4"):
        if const_input_layer:
            with tf.variable_scope("Const"):
                x = tf.get_variable(
                    "const", shape=[1, nf(1), 4, 4], initializer=tf.initializers.ones()
                )
                x = layer_epilogue(
                    tf.tile(tf.cast(x, dtype), [tf.shape(dlatents_in)[0], 1, 1, 1]), 0
                )
        else:
        ...
        with tf.variable_scope("Conv"):
            x = layer_epilogue(
                conv2d(x, fmaps=nf(1), kernel=3, gain=gain, use_wscale=use_wscale), 1
            )
    ...
    def block(res, x):  # res = 3..resolution_log2
        with tf.variable_scope("%dx%d" % (2 ** res, 2 ** res)):
            ...
            with tf.variable_scope("Conv1"):
                x = layer_epilogue(
                    conv2d(
                        x, fmaps=nf(res - 1), kernel=3, gain=gain, use_wscale=use_wscale
                    ),
                    res * 2 - 3,
                )
            return x
    ...
    def torgb(res, x):  # res = 2..resolution_log2
        ...
        with tf.variable_scope("ToRGB_lod%d" % lod):
            return apply_bias(
                conv2d(x, fmaps=num_channels, kernel=1, gain=1, use_wscale=use_wscale)
            )
    ...
    if structure == "recursive":
        ...
        def grow(x, res, lod):
            ...
            img = cset(
                ...
                lambda: upscale2d(
                    tflib.lerp(
                        torgb(res, y), upscale2d(torgb(res - 1, x)), lod_in - lod
                    ),
                    2 ** lod,
                ),
            )
...
def D_basic(
    ...
    **_kwargs
    ...
    labels_in = tf.cast(labels_in, dtype)
    lod_in = tf.cast(
        tf.get_variable("lod", initializer=np.float32(0.0), trainable=False), dtype
    )
    scores_out = None
    ...
    def fromrgb(x, res):  # res = 2..resolution_log2
        with tf.variable_scope("FromRGB_lod%d" % (resolution_log2 - res)):
            return act(
                apply_bias(
                    conv2d(
                        x, fmaps=nf(res - 1), kernel=1, gain=gain, use_wscale=use_wscale
                    )
                )
            )
            ...
            if res >= 3:  # 8x8 and up
                ...
                with tf.variable_scope("Dense0"):
                    x = act(
                        apply_bias(
                            dense(
                                x, fmaps=nf(res - 2), gain=gain, use_wscale=use_wscale
                            )
                        )
                    )
                with tf.variable_scope("Dense1"):
                    x = apply_bias(
                        dense(
                            x, fmaps=max(label_size, 1), gain=1, use_wscale=use_wscale
                        )
                    )
            return x

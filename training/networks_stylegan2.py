...
from dnnlib.tflib.ops.upfirdn_2d import (
    conv_downsample_2d,
    downsample_2d,
    upsample_2d,
    upsample_conv_2d,
...
def conv2d_layer(
    ...
    if up:
        x = upsample_conv_2d(
            x, tf.cast(w, x.dtype), data_format="NCHW", k=resample_kernel
        )
    elif down:
...
def modulated_conv2d_layer(
    ...
    # Modulate.
    s = dense_layer(
        y, fmaps=x.shape[1].value, weight_var=mod_weight_var
    )  # [BI] Transform incoming W to style.
    s = apply_bias_act(s, bias_var=mod_bias_var) + 1  # [BI] Add bias (initially 1).
    ww *= tf.cast(
        s[:, np.newaxis, np.newaxis, :, np.newaxis], w.dtype
    )  # [BkkIO] Scale input feature maps.
    ...
    if demodulate:
        ...
        ]  # [BkkIO] Scale output feature maps.

    # Reshape/scale input.
    if fused_modconv:
        x = tf.reshape(
            x, [1, -1, x.shape[2], x.shape[3]]
        ...
        x *= tf.cast(
            s[:, :, np.newaxis, np.newaxis], x.dtype
    ...
    if fused_modconv:
        ...
        x *= tf.cast(
            d[:, :, np.newaxis, np.newaxis], x.dtype
...
def G_main(
    ...
    **kwargs
):  # Arguments for sub-networks (mapping and synthesis).
    ...
    dlatents = components.mapping.get_output_for(
        latents_in, labels_in, is_training=is_training, **kwargs
    )
    ...
    if truncation_psi is not None:
        with tf.variable_scope("Truncation"):
            ...
            if truncation_cutoff is None:
                ...
                layer_psi = tf.where(
                    layer_idx < truncation_cutoff, layer_psi * truncation_psi, layer_psi
                )
    ...
    **_kwargs
    ...
    for layer_idx in range(mapping_layers):
        with tf.variable_scope("Dense%d" % layer_idx):
            ...
            x = apply_bias_act(
                dense_layer(x, fmaps=fmaps, lrmul=mapping_lrmul),
                act=act,
                lrmul=mapping_lrmul,
            )
    ...
    **_kwargs
    ...
    def layer(x, layer_idx, fmaps, kernel, up=False):
        ...
        noise_strength = tf.get_variable(
            "noise_strength", shape=[], initializer=tf.initializers.zeros()
        )
    ...
    def block(res, x):  # res = 3..resolution_log2
        with tf.variable_scope("%dx%d" % (2 ** res, 2 ** res)):
            with tf.variable_scope("Conv0_up"):
                x = layer(
                    x, layer_idx=res * 2 - 5, fmaps=nf(res - 1), kernel=3, up=True
                )
    ...
    def block(x, res):  # res = 2..resolution_log2
        with tf.variable_scope("%dx%d" % (2 ** res, 2 ** res)):
            with tf.variable_scope("Conv0"):
                x = apply_bias_act(
                    conv2d_layer(x, fmaps=nf(res - 1), kernel=3), act=act
                )

...
def G_logistic(G, D, opt, training_set, minibatch_size):
    ...
    loss = -tf.nn.softplus(
        fake_scores_out
    )  # log(1-sigmoid(fake_scores_out)) # pylint: disable=invalid-unary-operand-type
    return loss, None
...
def D_logistic(G, D, opt, training_set, minibatch_size, reals, labels):
    ...
    loss += tf.nn.softplus(
        -real_scores_out
    )  # -log(sigmoid(real_scores_out)) # pylint: disable=invalid-unary-operand-type
    return loss, None
...
def D_wgan_gp(
    ...
    with tf.name_scope("GradientPenalty"):
        ...
        mixed_grads = tf.gradients(tf.reduce_sum(mixed_scores_out), [mixed_images_out])[
            0
        ]
...
def G_logistic_ns_pathreg(
    ...
    fake_images_out, fake_dlatents_out = G.get_output_for(
        latents, labels, is_training=True, return_dlatents=True
    )
    fake_scores_out = D.get_output_for(fake_images_out, labels, is_training=True)
    loss = tf.nn.softplus(-fake_scores_out)  # -log(sigmoid(fake_scores_out))
    ...
    with tf.name_scope("PathReg"):
        ...
        pl_grads = tf.gradients(
            tf.reduce_sum(fake_images_out * pl_noise), [fake_dlatents_out]
        )[0]
        pl_lengths = tf.sqrt(
        ...
        with tf.control_dependencies(None):
            pl_mean_var = tf.Variable(
                name="pl_mean", trainable=False, initial_value=0.0, dtype=tf.float32
            )

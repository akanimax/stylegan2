...
def process_reals(x, labels, lod, mirror_augment, drange_data, drange_net):
    ...
    if mirror_augment:
        with tf.name_scope("MirrorAugment"):
            x = tf.where(
                tf.random_uniform([tf.shape(x)[0]]) < 0.5, x, tf.reverse(x, [3])
            )
    with tf.name_scope(
        "FadeLOD"
...
def training_schedule(
    ...
    tick_kimg_dict={8: 28, 16: 24, 32: 20, 64: 16, 128: 12, 256: 8, 512: 6, 1024: 4},
...
def training_loop(
    ...
    with tf.name_scope("Inputs"), tf.device("/cpu:0"):
        ...
        Gs_beta = (
            0.5
            ** tf.div(tf.cast(minibatch_size_in, tf.float32), G_smoothing_kimg * 1000.0)
        ...
        with tf.name_scope("GPU%d" % gpu), tf.device("/gpu:%d" % gpu):
            ...
            with tf.name_scope("DataFetch"):
                ...
                reals_var = tf.Variable(
                    name="reals",
                    trainable=False,
                    initial_value=tf.zeros([sched.minibatch_gpu] + training_set.shape),
                )
            ...
            if not lazy_regularization:
                ...
                if G_reg is not None:
                    G_reg_opt.register_gradients(
                        tf.reduce_mean(G_reg * G_reg_interval), G_gpu.trainables
                    )
                if D_reg is not None:
                    D_reg_opt.register_gradients(
                        tf.reduce_mean(D_reg * D_reg_interval), D_gpu.trainables
                    )
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        if cur_tick < 0 or cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            if image_snapshot_ticks is not None and (
                ...
                misc.save_image_grid(
                    grid_fakes,
                    dnnlib.make_run_dir_path("fakes%06d.png" % (cur_nimg // 1000)),
                    drange=drange_net,
                    grid_size=grid_size,
                )
            ...
            dnnlib.RunContext.get().update(
                "%.2f" % sched.lod, cur_epoch=cur_nimg // 1000, max_epoch=total_kimg
            )
            maintenance_time = (

...
class TFRecordDataset:
    def __init__(
        ...
        tfr_lods = [
            self.resolution_log2 - int(np.log2(shape[1])) for shape in tfr_shapes
        ]
        ...
        with tf.name_scope("Dataset"), tf.device("/cpu:0"):
            ...
            self._tf_iterator = tf.data.Iterator.from_structure(
                self._tf_datasets[0].output_types, self._tf_datasets[0].output_shapes
            )
            self._tf_init_ops = {
    ...
    def parse_tfrecord_np(record):
        ...
        shape = ex.features.feature[
            "shape"
        ].int64_list.value  # pylint: disable=no-member
